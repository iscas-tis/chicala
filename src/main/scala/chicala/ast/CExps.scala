...
trait CExps { self: ChicalaAst =>
  ...
  sealed abstract class CUnaryOp                                   extends COp
  sealed abstract class CBinaryOp(val left: CExp, val right: CExp) extends COp
  sealed abstract class CTernaryOp                                 extends COp

  case class Not(inner: CExp)                                         extends CUnaryOp
  ...
  case class Mux(condition: CExp, ifTrue: CExp, ifFalse: CExp)        extends CTernaryOp
